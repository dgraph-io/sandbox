...
This branch demonstrates using custom JWT claims along with ACL for query and mutation authentication in a GraphQL-based Dgraph **on-prem** cluster with illustrative Python clients.

### 2. Launch a local cluster (see steps overview above)

Note, there's a screen recording that [demos all this](videos/on-prem-acl-gql-auth-plus-python.mp4)
